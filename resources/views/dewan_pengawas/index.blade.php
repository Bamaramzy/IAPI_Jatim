<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Manajemen Dewan Pengawas Nasional') }}
        </h2>
    </x-slot>
    ...
    <div class="max-w-6xl mx-auto sm:px-6 lg:px-8 mt-6">
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6">
            ...
            <!-- Tombol Tambah -->
            <div class="mb-4">
                <a href="{{ route('dewan_pengawas.create') }}"
                        ...
                        @forelse ($pengawas as $item)
                            <tr class="border-b dark:border-gray-600">
                                ...
                                <td class="px-4 py-2 flex space-x-2">
                                    <!-- Edit -->
                                    <a href="{{ route('dewan_pengawas.edit', $item->id) }}"
                                        class="px-3 py-1 bg-yellow-500 text-white rounded-md hover:bg-yellow-600 text-xs">
                                        Edit
                                    </a>
                                    <!-- Hapus -->
                                    <form action="{{ route('dewan_pengawas.destroy', $item->id) }}" method="POST"
                                        onsubmit="return confirm('Yakin ingin menghapus data pengawas ini?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
