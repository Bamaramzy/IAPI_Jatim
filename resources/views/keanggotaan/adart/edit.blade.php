    ...
    <div class="py-6">
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 shadow rounded-lg p-6">
                ...
                <form action="{{ route('adart.update', $adart->id) }}" method="POST" enctype="multipart/form-data"
                    class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        @if ($adart->cover)
                            <div class="mb-2">
                                <img src="{{ asset('storage/' . $adart->cover) }}" alt="Cover"
                    ...
                    <div>
                        ...
                        <select name="status" class="block w-full border rounded-md p-2 text-sm">
                            <option value="aktif" {{ old('status', $adart->status) == 'aktif' ? 'selected' : '' }}>
                                Aktif</option>
                            <option value="nonaktif"
                                {{ old('status', $adart->status) == 'nonaktif' ? 'selected' : '' }}>
                                Nonaktif</option>
                        </select>
                        @error('status')
                            <p class="text-red-600 text-sm">{{ $message }}</p>
