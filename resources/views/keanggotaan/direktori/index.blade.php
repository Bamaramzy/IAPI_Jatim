                ...
                <table class="min-w-full border border-gray-200 dark:border-gray-700">
                    ...
                    <tbody>
                        @forelse ($direktoris as $d)
                            @php
                                $driveId = null;
                                if ($d->link_drive && preg_match('/[-\w]{25,}/', $d->link_drive, $matches)) {
                                    $driveId = $matches[0];
                                }
                            @endphp
                                    ...
                                    @if ($driveId)
                                        <a href="{{ $d->link_drive }}" target="_blank">
                                            <img src="https://drive.google.com/thumbnail?id={{ $driveId }}"
                                                alt="Preview Drive" class="w-16 h-16 object-cover mx-auto rounded">
                                        </a>
                                    @elseif ($d->file_pdf)
                                        <a href="{{ asset('storage/' . $d->file_pdf) }}" target="_blank">
                                            <img src="https://drive.google.com/thumbnail?id={{ basename($d->file_pdf) }}"
                                                alt="Preview PDF" class="w-16 h-16 object-cover mx-auto rounded">
                                        </a>
                                    @elseif ($d->cover)
